...
public class CollectionManager {
    ...
    public boolean removeByID(Long id){
        long n = 0;
        n = listCity.stream().filter(p -> p.getId().equals(id)).count();

        listCity.stream().filter(p -> p.getId().equals(id)).forEach(p -> {listCity.remove(p);
                                                                            });
        if (n == 0){
            return false;
        }return true;
...
//            }
    ...
    public void removeGreater(City c){
        long n = 0;
        n = listCity.stream().filter(p -> c.compareTo(p) < 0).count();
        listCity.stream().filter(p -> c.compareTo(p) < 0).forEach(p -> {ResponseOutput.appendln("Был удален элемент с id = "+ p.getId());
                                                                        listCity.remove(p);
                                                                        });
